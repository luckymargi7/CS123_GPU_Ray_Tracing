...
: _nx( 0u ), _ny( 0u ), _raster( 0 )
{
  ...
  try {
    ...
    inf >> minor >> _ny >> major >> _nx;
    if(minor != "-Y" || major != "+X") throw "Can only handle -Y +X ordering";
    if(_nx <= 0 || _ny <= 0) throw "Invalid image dimensions";
    getLine(inf, comment); // Read the last newline of the header

    RGBe *RGBERaster = new RGBe[_nx * _ny];

    for(unsigned int y=0; y<_ny; y++) {
      ReadScanline(inf, RGBERaster + _nx*y, _nx);
    }

    _raster = new float[_nx * _ny * 4];

    float inv_img_exposure = 1.0f / exposure;
    for(unsigned int i=0; i<_nx*_ny; i++) {
      RGBEtoFloats(RGBERaster[i], _raster + i*4, inv_img_exposure);
    }
    delete[] RGBERaster;
