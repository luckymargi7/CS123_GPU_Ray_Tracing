...
void ObjLoader::createGeometryInstances( GLMmodel* model )
{
  ...
  if( !_bbox_program.get() ) {
    std::string path = std::string(sutilSamplesPtxDir()) + "/cuda_compile_ptx_generated_triangle_mesh.cu.ptx";
    _bbox_program = _context->createProgramFromPTXFile( path, "mesh_bounds" );
  }
  ...
  for ( GLMgroup* obj_group = model->groups;
    ...
    if (_large_geom) {
      ...
      float* vertex_buffer_data = static_cast<float*>( _vbuffer->map() );
      RTsize nverts;
      _vbuffer->getSize(nverts);
      ...
      mesh = _context->createGeometry();
      mesh->setPrimitiveCount( num_triangles );
      mesh->setIntersectionProgram( _intersect_program);
      mesh->setBoundingBoxProgram( _bbox_program );
  ...
  if (_large_geom) {
    ...
    if ( _ASBuilder == std::string("Sbvh") || _ASBuilder == std::string("TriangleKdTree") || _ASTraverser == std::string( "KdTree" )) {
