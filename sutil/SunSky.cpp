...
PreethamSunSky::PreethamSunSky()
  : _sun_theta( static_cast<float>( M_PI ) ),
    ...
    _dirty( true )
{
  _up = make_float3( 0.0f, 1.0f, 0.0f );
}


void PreethamSunSky::preprocess()
{
  if( !_dirty ) return;
  ...
  _c0 = make_float3 (  0.1787f * _turbidity - 1.4630f,
                      -0.0193f * _turbidity - 0.2592f,
                      -0.0167f * _turbidity - 0.2608f );

  _c1 = make_float3 ( -0.3554f * _turbidity + 0.4275f,
                      -0.0665f * _turbidity + 0.0008f,
                      -0.0950f * _turbidity + 0.0092f );

  _c2 = make_float3 ( -0.0227f * _turbidity + 5.3251f,
                      -0.0004f * _turbidity + 0.2125f,
                      -0.0079f * _turbidity + 0.2102f );

  _c3 = make_float3 (  0.1206f * _turbidity - 2.5771f,
                      -0.0641f * _turbidity - 0.8989f,
                      -0.0441f * _turbidity - 1.6537f );

  _c4 = make_float3 ( -0.0670f * _turbidity + 0.3703f,
  ...
  float3 divisor_Yxy = ( 1.0f + _c0 * expf(_c1) ) * 
                       ( 1.0f + _c2 * expf(_c3*_sun_theta) + _c4 * cos_sun_theta * cos_sun_theta );
  ...
  float sin_sun_theta = sinf( _sun_theta );
  _sun_dir = make_float3( cosf( _sun_phi ) * sin_sun_theta,
                          sinf( _sun_phi ) * sin_sun_theta,
                          cosf( _sun_theta ) ); 
  optix::Onb onb( _up );
...
void PreethamSunSky::setVariables(optix::Context context )
{
  preprocess();

  context["c0"]->setFloat(_c0);
  context["c1"]->setFloat(_c1);
  context["c2"]->setFloat(_c2);
  context["c3"]->setFloat(_c3);
  context["c4"]->setFloat(_c4);
  context["inv_divisor_Yxy"]->setFloat( _inv_divisor_Yxy );

  context["sky_up"  ]->setFloat( _up );
  context["overcast"]->setFloat( _overcast );

  context["sun_direction"]->setFloat( _sun_dir );
  context["sun_color"    ]->setFloat( _sun_color );
}
...
float3 PreethamSunSky::skyColor( const float3 & direction, bool CEL )
{
  ...
  if( _overcast < 1.0f ) {
    ...
    if( CEL  && dot( ray_direction, _sun_dir ) > 94.0f / sqrtf( 94.0f*94.0f + 0.45f*0.45f) ) {
      ...
      float3 A =  _c1 * inv_dir_dot_up;
      float3 B =  _c3 * acos_gamma;
      float3 color_Yxy = ( make_float3(1.0f) + _c0*make_float3( expf(A.x), expf(A.y), expf(A.z) ) ) *
                         ( make_float3(1.0f) + _c2*make_float3( expf(B.x), expf(B.y), expf(B.z) ) + _c4*gamma*gamma );
  ...
  return lerp( sunlit_sky_color, overcast_sky_color, _overcast );
}
