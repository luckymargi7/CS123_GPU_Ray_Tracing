...
MeshScene::MeshScene():
  _accel_builder("Sbvh"),
  _accel_traverser("Bvh"),
  _accel_cache_loaded( false ),
  _accel_caching_on  ( false ),
  _accel_large_mesh  ( false )
{
}
...
SUTILAPI MeshScene::MeshScene(bool accel_cache_loaded, bool accel_caching_on, bool accel_large_mesh):
  _accel_builder("Sbvh"),
  _accel_traverser("Bvh"),
  _accel_cache_loaded( accel_cache_loaded ),
  _accel_caching_on  ( accel_caching_on ),
  _accel_large_mesh  ( accel_large_mesh )
{
}
...
void MeshScene::loadAccelCache()
{
  ...
  if( _accel_caching_on ) {
    ...
    if ( in ) {
      ...
      Acceleration accel = _geometry_group->getAcceleration();
      try {
        accel->setData( data, static_cast<RTsize>(size) );
        _accel_cache_loaded = true;
...
void MeshScene::saveAccelCache()
{
  ...
  if( _accel_caching_on && !_accel_cache_loaded ) {
...
{
  std::string cachefile = _filename;
