/* We are using this class almost exactly as it was provided because
   it takes care a of a lot of basic functionality that can only be
   done right one way.
 */
...
Scene::InitialCameraData::InitialCameraData( const std::string &camstr)
{
  std::istringstream istr(camstr);
  istr >> eye >> lookat >> up >> vfov;
}

const char* const Scene::ptxpath( const std::string& target, const std::string& base )
{
...
Buffer Scene::createOutputBuffer( RTformat format,
                                        unsigned int width,
                                        unsigned int height )
{
  Buffer buffer;
