...
//! Data for a simple primitive.
struct SimplePrimitive
{
   PrimitiveType type;
   CS123SceneMaterial material;
   CS123SceneFileMap filemap;
   Matrix4x4 transMat;

};

//! A point or vector (and a primitive on which the point may be located
struct DividedPoint
{
   Vector4 P;  //! point of origination
   Vector4 D;  //! direction of vector
   double t;  //! location along vector
   SimplePrimitive prim; //! primitive that contains this point
   Vector4 normal; //! normal at this point
};
